...
typedef struct {
    ...
    XBinary_threshold_uio_map maps[ MAX_UIO_MAPS ];
} XBinary_threshold_uio_info;

/***************** Variable Definitions **************************************/
static XBinary_threshold_uio_info uio_info;
...
static int uio_info_read_name(XBinary_threshold_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBinary_threshold_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBinary_threshold_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBinary_threshold_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBinary_threshold_Initialize(XBinary_threshold *InstancePtr, const char* InstanceName) {
	XBinary_threshold_uio_info *InfoPtr = &uio_info;
